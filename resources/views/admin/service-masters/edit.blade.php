        ...
        <div class="content-body">
            <form method="POST" id="catalogForm" enctype="multipart/form-data">
                ...
                <div class="pa-catalog-toolbar">
                    <div>
                        <h2>Edit Premium Catalog</h2>
                        <p>Updating service details for Luxury Service: #{{ $service->id }}</p>
                    </div>
                ...
                <div class="row pa-catalog-layout">
                    <div class="col-xl-5">
                        ...
                        <div class="card shadow-sm border-0 pa-catalog-card" id="variants-section" style="{{ $service->has_variants ? '' : 'display: none;' }}">
                                ...
                                @foreach($service->variants ?? [] as $vIdx => $variant)
                                    <div class="variant-row border p-2 mb-2 rounded bg-white shadow-sm position-relative">
                                        ...
                                        <div class="row g-1">
                                            ...
                                            <div class="col-md-6 mb-1"><label class="form-label small">Discount %</label><div class="input-group input-group-sm"><input type="number" step="0.01" name="variants[{{$vIdx}}][discount_percentage]" class="form-control" value="{{ $variant->discount_percentage }}" placeholder="e.g. 10"><span class="input-group-text">%</span></div></div>
                                            ...
                                            <div class="col-md-3 mb-1"><label class="form-label small">Rating</label><input type="number" step="0.1" min="0" max="5" name="variants[{{$vIdx}}][rating]" class="form-control form-control-sm" value="{{ $variant->rating }}" placeholder="4.5"></div>
                                            <div class="col-md-3 mb-1"><label class="form-label small">Reviews #</label><input type="number" name="variants[{{$vIdx}}][reviews]" class="form-control form-control-sm" value="{{ $variant->reviews }}" placeholder="150"></div>
                                            <div class="col-md-12 mb-1">
                                                <label class="form-label small">Thumbnail Image</label>
                                                <input type="hidden" name="variants[{{$vIdx}}][old_thumbnail_image]" value="{{ $variant->thumbnail_image }}">
                                                <div class="variant-thumb-upload" style="border:2px dashed #d1d5db;border-radius:8px;padding:8px;text-align:center;cursor:pointer;position:relative;">
                                                    <p class="mb-0 small text-muted"><i data-feather="image" style="width:14px"></i> {{ $variant->thumbnail_image ? 'Change Image' : 'Upload Thumbnail' }}</p>
                                                    <input type="file" name="variants[{{$vIdx}}][thumbnail_image]" accept="image/*" style="position:absolute;top:0;left:0;width:100%;height:100%;opacity:0;cursor:pointer;" onchange="handleVariantThumbPreview(this)">
                                                </div>
                                                <div class="variant-thumb-preview mt-1" style="{{ $variant->thumbnail_image ? 'display:flex;align-items:center;gap:8px;' : 'display:none;' }}">
                                                    <img src="{{ $variant->thumbnail_image ? asset('uploads/service-variant/' . $variant->thumbnail_image) : '' }}" style="max-height:60px;border-radius:6px;border:1px solid #ddd;">
                                                    <button type="button" class="btn btn-sm btn-link text-danger p-0 clear-variant-thumb">Remove</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    window.handleVariantThumbPreview = function(input) {
        ...
        reader.onload = function(e) {
            ...
            previewDiv.css('display', 'flex').css('align-items', 'center').css('gap', '8px');
        };
        reader.readAsDataURL(file);
    };
    ...
    $(document).on('click', '.clear-variant-thumb', function() {
        ...
        row.find('input[name*="old_thumbnail_image"]').val('');
    });
    ...
    $(function() {
        ...
        $('.add-variant').click(function() {
            ...
            html += '<button type="button" class="btn btn-sm text-danger position-absolute top-0 end-0 remove-row">&times;</button>';
