                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select name="service_type_id" class="form-control select2" required>
                                                        ...
                                                        @foreach($serviceTypes as $type)
                                                            <option value="{{ $type->id }}" {{ $combo->service_type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                                        @endforeach
                                                    </select>
                                                    <div class="valid-feedback"></div>
                                                </div>
                                                                            ...
                                                                            @foreach($svcs as $service)
                                                                                @if($service->variants->count() > 0)
                                                                                    @foreach($service->variants as $variant)
                                                                                        ...
                                                                                        <div class="service-item-row" data-search="{{ strtolower($service->name . ' ' . $variant->name . ' ' . $catName . ' ' . $subCatName) }}">
                                                                                            <input type="checkbox" name="services[]" value="{{ $val }}" class="service-checkbox custom-service-checkbox" id="svc_{{ $service->id }}_var_{{ $variant->id }}" {{ in_array($val, $selectedItems) ? 'checked' : '' }}>
                                                                                ...
                                                                                @else
                                                                                    ...
                                                                                    <div class="service-item-row" data-search="{{ strtolower($service->name . ' ' . $catName . ' ' . $subCatName) }}">
                                                                                        <input type="checkbox" name="services[]" value="{{ $val }}" class="service-checkbox custom-service-checkbox" id="svc_{{ $service->id }}" {{ in_array($val, $selectedItems) ? 'checked' : '' }}>
                                                                ...
                                                                @foreach($service->variants as $variant)
                                                                    ...
                                                                    @if(in_array($val, $selectedItems))
                                                                        <div class="form-check mb-1">
                                                                            ...
                                                                            <label class="form-check-label" for="def_{{ $val }}">{{ $service->name }} <span class="variant-badge">{{ $variant->name }}</span></label>
                                                                        </div>
                                                                    @endif
                                                                @endforeach
                                                            @else
    ...
    $(document).ready(function() {
        ...
        function updateDefaultServicesList() {
            ...
            $('.custom-service-checkbox:checked').each(function() {
                ...
                let oldEl = $(document.getElementById(`def_${val}`));
                let isChecked = (oldEl.length > 0 && !oldEl.prop('checked')) ? '' : 'checked';
                
                listHtml += `
                    <div class="form-check mb-1">
                        <input class="form-check-input" type="checkbox" name="default_services[]" value="${val}" id="def_${val}" ${isChecked}>
