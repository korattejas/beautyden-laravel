...
<style>
    ...
    .stats-card {
        ...
        box-shadow: var(--mst-shadow); transition: transform 0.3s;
    }
    .stats-card:hover { transform: translateY(-5px); }
    .stats-card i { font-size: 2rem; color: var(--mst-primary); margin-bottom: 10px; }
...
<div class="app-content content" style="padding: 0;">
        ...
        <div class="container-fluid px-5">
            <div class="row align-items-center">
                ...
                <div class="col-md-3 text-md-end">
                    <span class="badge {{ $user->status == 1 ? 'bg-success' : 'bg-danger' }} fs-5 px-3 py-2 rounded-pill">
    ...
    <div class="container-fluid px-5">
        ...
        <div class="row">
                            ...
                            <table class="table mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.appointments.view', $app->id) }}" class="btn btn-sm btn-outline-primary">View</a>
                                        </td>
                                    </tr>
                                    @empty
            ...
            <!-- Right Side: Memberships & Details -->
            <div class="col-md-4">
                    ...
                    <div class="card-body">
                        @if($active_plan = $user->activeSubscription())
                            <div class="alert alert-success d-flex align-items-center">
                                ...
                                <div>
                                    <h6 class="alert-heading mb-0">{{ $active_plan->plan->name }}</h6>
                                    <small>Valid until {{ $active_plan->end_date }}</small>
                                </div>
                            </div>
                            <p class="mb-1 text-muted">Started on: {{ $active_plan->start_date }}</p>
                            <p class="mb-0 text-muted">Price Paid: ₹{{ $active_plan->price_paid }}</p>
                        @else
                            <div class="text-center py-3">
                                <i class="bi bi-x-circle text-danger fs-1"></i>
                                <p class="mt-2 text-muted">No active membership plan.</p>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="card-body p-0">
                        <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item">
                                <span class="badge bg-light text-dark mb-2">{{ ucfirst($addr->address_type ?? 'Home') }}</span>
                                <p class="small mb-0">{{ $addr->address }}, {{ $addr->area }}</p>
                            </li>
                            @empty
                            <li class="list-group-item text-center text-muted py-3">No addresses saved.</li>
                            @endforelse
                        </ul>
                    </div>
                </div>
